...
const MovieItem = ({ id, posterPhoto, title, voteAvg,horizontal =false,overview }) =>
  horizontal?<HContainer>
      <MoviePoster path={posterPhoto} />
      <Column>
      <Title big={true}>{title}</Title>
      <MovieRating votes={voteAvg} />
      {overview?
      <Overview>{overview.length > 130?`${overview.substring(0,127)}...`:overview}</Overview>:null
    }
      </Column>
  </HContainer>: (
    <Container>
      <MoviePoster path={posterPhoto} />
      <Title>{title.length>10?`${title.substring(0,9)}...`:title}</Title>
      <MovieRating votes={voteAvg} />
    </Container>
  );
